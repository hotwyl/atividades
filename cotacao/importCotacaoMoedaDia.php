...
function getFirstAndLastDayOfMonth($dateStr) {
    ...
    return [
        'start_date' => $newDate->format('m-d-Y'),
        'end_date' => $newDate->format('m-d-Y'),
    ];
}
...
function saveToJson($data, $endDate) {
    ...
    if(count($data)<=0){
        $data['message'] = 'não encontrado dados de cotação para esta data';
    }
...
foreach ($currencyTypes['value'] as $currency) {
    ...
    $ratesNextDay = getCurrencyRatesNextMonth($symbol, $startDate, $endDate);
    
    if(count($ratesNextDay['value']) > 0 && !in_array($ratesNextDay['value'], [null, false, '', []])){
        $arrRates[$currency['simbolo']] = $ratesNextDay['value'];
    }
